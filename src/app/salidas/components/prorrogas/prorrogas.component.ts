...
import { dejarNumeroBrutos, formateadorMiles, formateadorMilesSinDecimal, formatearNumeroBrutosAMiles } from 'src/app/shared/utils/formateadores';
...
export class ProrrogasComponent {
  ...
  @ViewChild('modalProrrogaAgregarEditar') modalProrrogaAgregarEditar!: ModalComponent;
  @ViewChild('modalProrrogaVizualizar') modalProrrogaVizualizar!: ModalComponent;
      ...
      .subscribe({
        next: (response) => {
          ...
          if (!this.prorrogas.length) {
            ...
            let obtenerUltimaProrroga = this.prorrogas.find(prorroga => prorroga.estado);
            if (obtenerUltimaProrroga) {
              ...
              this.formAgregarProrroga.patchValue({
                ...
                fechaVencimientoActual: obtenerUltimaProrroga.fechaNuevaVencimiento,
                saldoCapital: formatearNumeroBrutosAMiles(obtenerUltimaProrroga.saldoCapitalActualizado),
                valorUfHoy: formatearNumeroBrutosAMiles(this.valorUfHoy),
                valorUfFechaCurse: formatearNumeroBrutosAMiles(obtenerUltimaProrroga.valorUfHoy),
                fechaFirmaCurse: obtenerUltimaProrroga.fechaCurseActual
              });
            } else {
  ...
  obtenerDetalleClienteSalida() {
    this.sSalidas.obtenerClienteSalidaDetalle().pipe(
      switchMap(response => {
        const fecha = response.data.fecha_cursado_inicio;

        return forkJoin({
          detalle: of(response),
          ufFecha: this.sValorUf.obtenerValorUfFechaEspecifica(fecha.toString())
        });
      })
    ).subscribe({
      next: ({ detalle, ufFecha }) => {
        ...
        this.formAgregarProrroga.patchValue({
          ...
          saldoCapital: formatearNumeroBrutosAMiles(detalle.data.valor_contrato),
          valorUfHoy: formatearNumeroBrutosAMiles(this.valorUfHoy),
          valorUfFechaCurse: formatearNumeroBrutosAMiles(this.valorUfFechaEspecifica),
    ...
    this.sSalidas.obtenerProrrogaPorCliente()
      .subscribe({
        next: (response) => {
          ...
          let prrorogaPendiente = this.prorrogas.some(prorroga => !prorroga.estado);
          if (prrorogaPendiente) { this.simulacionPendiente = true; }
  ...
  obtenerValorUfHoy() {
    this.sValorUf.obtenerValorUfHoy()
      .subscribe({
        next: (resp) => {
  ...
  formateadorMiles(event: Event) {
    ...
    this.formAgregarProrroga.get(input.getAttribute('formControlName')!)?.setValue(input.value)
  }
  ...
  calcularSaldoCapitarActualizado() {
    ...
    let saldoCapital = formulario.value.saldoCapital ? Number(dejarNumeroBrutos(formulario.value.saldoCapital)) : 0;
    let abonoCapital = formulario.value.abonoCapital ? Number(dejarNumeroBrutos(formulario.value.abonoCapital)) : 0;
  ...
  calcularPorcentajeComisionProrroga() {
    let formulario = this.formAgregarProrroga;
    let saldoCapitalActualizado = formulario.value.saldoCapitalActualizado ? Number(dejarNumeroBrutos(formulario.value.saldoCapitalActualizado)) : 0;
    let comisionEnPesos = formulario.value.comisionEnPesos ? Number(dejarNumeroBrutos(formulario.value.comisionEnPesos)) : 0;

    if (!comisionEnPesos || !saldoCapitalActualizado || !this.valorUfHoy) {
      return formulario.patchValue({ comisionEnPorcentaje: 0 })
    }

    let resultado = (comisionEnPesos / this.valorUfHoy) / saldoCapitalActualizado * 100;
  ...
  calcularValorSeguro() {
    let formulario = this.formAgregarProrroga;
    let valorAnual = formulario.value.seguroValorAnual ? Number(dejarNumeroBrutos(formulario.value.seguroValorAnual)) : 0;
    let cantidadMeses = formulario.value.seguroCantidadMeses ? Number(dejarNumeroBrutos(formulario.value.seguroCantidadMeses)) : 0;
  ...
  calcularValorContribucion() {
    let formulario = this.formAgregarProrroga;
    let valorAnual = formulario.value.contribucinesValorAnual ? Number(dejarNumeroBrutos(formulario.value.contribucinesValorAnual)) : 0;
    let cantidadMeses = formulario.value.contribucionesCantidadMeses ? Number(dejarNumeroBrutos(formulario.value.contribucionesCantidadMeses)) : 0;
  ...
  calcularSimulacionFinal() {
    ...
    this.detalleProrroga = {
      ...
      comisionProrroga: (this.formatearNumero(formulario.comisionEnPorcentaje) / 100) * this.formatearNumero(formulario.saldoCapitalActualizado),
      contribucionesProrroga: this.formatearNumero(formulario.contribucionesValorMensual),
      seguroProrroga: this.formatearNumero(formulario.seguroValorMensual),
      arriendoProrroga: this.formatearNumero(formulario.saldoCapitalActualizado) * Number(formulario.mesesProrroga) * (this.formatearNumero(formulario.valorTasaCliente) / 100),
    ...
    this.detalleProrroga = {
      ...
      total: this.detalleProrroga.comisionProrroga +
        ...
        (this.formatearNumero(formulario.saldoCapital) / this.formatearNumero(formulario.abonoCapital))
    }
  }

  formatearNumero(valor: any) { return Number(dejarNumeroBrutos(valor)); }
  ...
  agregarNuevaProrroga() {
    this.sSalidas.agregarProrrogaPorCliente(this.formAgregarProrroga.value)
      .subscribe({
        next: (response) => {
          if (response.ok) {
            mostrarMensaje({
  ...
  modalEditarProrroga(prorroga: Prorroga) {
    ...
    this.formAgregarProrroga.patchValue({
      ...
      saldoCapitalActualizado: formatearNumeroBrutosAMiles(prorroga.saldoCapitalActualizado),
      ...
      comisionEnPorcentaje: formatearNumeroBrutosAMiles(prorroga.comisionEnPorcentaje),
      seguroValorAnual: formatearNumeroBrutosAMiles(prorroga.seguroValorAnual),
      seguroCantidadMeses: formatearNumeroBrutosAMiles(prorroga.seguroCantidadMeses),
      seguroValorMensual: formatearNumeroBrutosAMiles(prorroga.seguroValorMensual),
      contribucinesValorAnual: formateadorMilesSinDecimal(prorroga.contribucinesValorAnual),
      contribucionesCantidadMeses: formatearNumeroBrutosAMiles(prorroga.contribucionesCantidadMeses),
      contribucionesValorMensual: formatearNumeroBrutosAMiles(prorroga.contribucionesValorMensual),
      rentasValorEnPesos: formateadorMilesSinDecimal(prorroga.rentasValorEnPesos),
      fechaFirmaCurse: prorroga.fechaFirmaCurse,
      valorUfFechaCurse: formatearNumeroBrutosAMiles(prorroga.valorUfFechaCurse),
      valorRentaCurse: formatearNumeroBrutosAMiles(prorroga.valorRentaCurse),
      valorTasaContrato: formatearNumeroBrutosAMiles(prorroga.valorTasaContrato),
      ...
      sobretasaProrroga: formatearNumeroBrutosAMiles(prorroga.sobretasaProrroga),
      gastosOperacionales: formatearNumeroBrutosAMiles(prorroga.gastosOperacionales),
  ...
  editarProrroga() {
    this.sSalidas.modificarProrrogaPorCliente(this.formAgregarProrroga.value)
      .subscribe({
        next: (response) => {
          if (response.ok) {
            mostrarMensaje({
  ...
  modalDetalleProrroga(prorroga: Prorroga) {
    ...
    this.detalleProrroga = {
      ...
      comisionProrroga: (this.formatearNumero(prorroga.comisionEnPorcentaje) / 100) * this.formatearNumero(prorroga.saldoCapitalActualizado),
      contribucionesProrroga: this.formatearNumero(prorroga.contribucionesValorMensual),
      seguroProrroga: this.formatearNumero(prorroga.seguroValorMensual),
      arriendoProrroga: this.formatearNumero(prorroga.saldoCapitalActualizado) * Number(prorroga.mesesProrroga) * (this.formatearNumero(prorroga.valorTasaCliente) / 100),
  ...
  eliminarProrroga(id_prorroga: number) {
    mostrarConfirmacion('Confirmar Eliminación', '¿Está seguro de que desea eliminar esta prórroga? Esta acción no se puede deshacer.')
      .then((confirmed) => {
        if (confirmed) {
  ...
  validarProrrogaAceptada(prorroga: Prorroga): void {
    mostrarConfirmacion('Confirmar Acción', '¿Está seguro de que desea aceptar esta prórroga? Esta acción no se puede deshacer.')
      .then((confirmed) => {
        if (confirmed) {

          this.sSalidas.aceptarProrrogaPorCliente(prorroga.id_prorroga)
            .subscribe({
              next: (response) => {
                if (response.ok) {

                  // Actualizar el estado localmente para reflejar el cambio en la UI
                  prorroga.estado = true;
                  this.simulacionPendiente = false;;
  ...
  imprimirDetalle(tipo: 'cliente' | 'inversionista') {
    ...
    this.sSalidas.exportarFichaDetalleProrroga(this.id_prorroga_imprimir, tipo)
      .subscribe({
        next: (response) => {
