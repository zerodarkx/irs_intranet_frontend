...
} from 'src/app/shared/utils/validadores';
import {
  IconoSweetAlert,
  mostrarMensaje,
...
export class DetalleClienteComponent implements OnInit {
  ...
  selectTipoRechazo: { id_tipoRechazo: number; nombre_tipoRechazo: string }[] =
    [
      { id_tipoRechazo: 1, nombre_tipoRechazo: 'LTV' },
      { id_tipoRechazo: 2, nombre_tipoRechazo: 'PROPIEDAD NO CALIFICA' },
      { id_tipoRechazo: 3, nombre_tipoRechazo: 'CLIENTE NO CONTESTA' },
      { id_tipoRechazo: 4, nombre_tipoRechazo: 'CLIENTE DESISTE' },
      { id_tipoRechazo: 5, nombre_tipoRechazo: 'Otros' },
    ];
  ...
  miFormulario: FormGroup = this.fb.group({
    rut_cli: ['', [Validators.required, rutValidator]],
    nombre_cli: ['', [Validators.required, Validators.minLength(6)]],
    correo_cli: [
      '',
      [Validators.required, Validators.email, validarCorreoInstitucional],
    ],
    cod_telefono: ['', [Validators.required]],
    telefono_cli: ['', [Validators.required, soloNumerosFormulario]],
    f_ingreso_cli: ['2024-12-12'],
    f_contacto_cli: ['', [Validators.required]],
    rol_cli: ['', [Validators.required]],
    l_plataforma_cli: [[], [Validators.required]],
    l_negocio_cli: [[], [Validators.required]],
    tipo_propiedad: [[], [Validators.required]],
    selectRegion: [[], [Validators.required]],
    selectComuna: [[], [Validators.required]],
    direccion_cli: ['', [Validators.required]],
    valorComercial_cli: [0, [Validators.required, soloNumerosFormulario]],
    deudaEstimada_cli: [0, [Validators.required, soloNumerosFormulario]],
    mTerreno: [0, [Validators.required, soloNumerosFormulario]],
    mConstruidos: [0, [Validators.required, soloNumerosFormulario]],
    obs_cli: [''],
    selectEjecutivo: [[]],
    motivo_rechazo: [],
  ...
  formRechazo: FormGroup = this.fb.group({
    ...
    tipo_rechazo: [, Validators.required],
  });
  ...
  selectRegion() {
    this.sRegion.obtenerTodasLasRegiones().subscribe({
      next: ({ data }) => {
  ...
  selectPlataforma() {
    this.sPlataforma.obtenerTodasLasPlataformas().subscribe({
      next: ({ data }) => {
  ...
  selectLineaNegocio() {
    this.sLineaNegocio.obtenerTodasLasLineasNegocio().subscribe({
      next: ({ data }) => {
  ...
  selectTipoPropiedad() {
    this.sTipoPropiedad.obtenerTodasLosTipoPropiedad().subscribe({
      next: ({ data }) => {
  ...
  selectEjecutivoBroker() {
    this.sUsuario.obtenerEjecutivosBrokers().subscribe({
      next: ({ data }) => {
  ...
  changeComuna(event: Iregiones | undefined): void {
    ...
    if (region) {
      this.sComuna.obtenerComunasPorRegion(region).subscribe({
        next: ({ data }) => {
  ...
  selectComunaPorRegionById(id_region: number) {
    this.sComuna.obtenerComunasPorRegion(id_region).subscribe({
      next: ({ data }) => {
        this.selectComunas = data;
      },
      error: (error: ErrorHttpCustom) => {
  ...
  obtenerCliente() {
    this.sCliente.obtenerClientePorId().subscribe({
      next: (response) => {
        ...
        if (response.ok) {
          this.miFormulario.patchValue({
            ...
            valorComercial_cli: formateadorMiles(
              response.data.cli_valor_comercial
            ),
            deudaEstimada_cli: formateadorMiles(
              response.data.cli_deuda_estimada
            ),
  ...
  guardarDatos(): void {
    this.sCliente.modificarClientePorId(this.miFormulario.value).subscribe({
      next: (response) => {
        mostrarMensaje({
          icono: response.ok
            ? IconoSweetAlert.Success
            : IconoSweetAlert.Warning,
  ...
  volverEstadoCliente() {
    this.sCliente.volverEstadoCliente().subscribe({
      next: (response) => {
        mostrarMensaje({
          icono: response.ok
            ? IconoSweetAlert.Success
  ...
  pasarSiguienteEstado() {
    this.sCliente.cambiarSiguienteEstado(this.miFormulario.value).subscribe({
      next: (response) => {
        mostrarMensaje({
          icono: response.ok
            ? IconoSweetAlert.Success
  ...
  rechazarCliente() {
    this.sCliente.rechazarCliente(this.formRechazo.value).subscribe({
      next: (response) => {
        mostrarMensaje({
