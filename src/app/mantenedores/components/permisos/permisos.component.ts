...
import {
  EstadoClientesService,
  TipoSimulacionCanalService,
...
export class PermisosComponent implements OnChanges {
  ...
  modulos: PermisosModulo[] = [
    {
      ...
      categorias: [
          ...
          subcategorias: [
            {
              ...
              activo: false,
              permiso: 'BTN_AGREGAR_CLIENTE',
            },
            {
              nombre: 'Exportar Excel',
              activo: false,
              permiso: 'BTN_EXPORTAREXCEL_CLIENTE',
            },
            {
              nombre: 'Filtrar Por Ejecutivo',
              activo: false,
              permiso: 'PER_FILTRAREJECUTIVO_CLIENTE',
            },
            {
              nombre: 'Rechazar Masivo',
              activo: false,
          ...
          subcategorias: [
            {
              nombre: 'Rechazar',
              activo: false,
              permiso: 'BTN_RECHAZAR_CLIENTE',
            },
            {
              nombre: 'Volver Estado',
              activo: false,
              permiso: 'BTN_VOLVER_CLIENTE',
            },
            {
              nombre: 'Guardar Cliente',
              activo: false,
              permiso: 'BTN_GUARDAR_CLIENTE',
            },
            {
              nombre: 'Siguiente Estado',
              activo: false,
              permiso: 'BTN_SIGUIENTE_CLIENTE',
            },
            {
              nombre: 'Asignar Ejecutivo',
              activo: false,
              permiso: 'PER_ASIGNAR_EJECUTIVO',
            },
          ],
          activo: false,
          ...
          subcategorias: [
            {
              ...
              activo: false,
              permiso: 'BTN_GUARDAR_DOCUMENTO',
            },
            {
              nombre: 'Eliminar Documento',
              activo: false,
              permiso: 'BTN_ELIMINAR_DOCUMENTO',
            },
            {
              nombre: 'Guardar Imagen',
              activo: false,
              permiso: 'BTN_GUARDAR_IMAGEN',
            },
            {
              nombre: 'Eliminar Imagen',
              activo: false,
              permiso: 'BTN_ELIMINAR_IMAGEN',
            },
          ],
          activo: false,
        ...
        },
        {
          ...
          subcategorias: [
            {
              nombre: 'Nueva Simulacion',
              activo: false,
              permiso: 'BTN_NUEVA_SIMULACION',
            },
            {
              nombre: 'Ver Simulacion',
              activo: false,
              permiso: 'BTN_VER_SIMULACION',
            },
            {
              nombre: 'Descargar Simulacion',
              activo: false,
              permiso: 'BTN_DESCARGAR_SIMULACION',
            },
            {
              nombre: 'Modificiar Comision',
              activo: false,
              permiso: 'PER_MODIFICAR_COMISION',
            },
            {
              nombre: 'Modificar Rentar',
              activo: false,
              permiso: 'PER_MODIFICAR_RENTA',
            },
          ],
          activo: false,
          ...
          subcategorias: [
            {
              nombre: 'Exportar Excel',
              activo: false,
              permiso: 'BTN_EXPORTAREXCEL_PROPIEDAD',
            },
            {
              nombre: 'Filtrar Por Ejecutivo',
              activo: false,
              permiso: 'PER_FILTRAREJECUTIVO_PROPIEDAD',
            },
          ],
        },
      ],
  ...
  obtenerSubcategorias(moduloIndex: number, categoriaIndex: number): FormArray {
    return this.obtenerCategorias(moduloIndex)
      .at(categoriaIndex)
      .get('subcategorias') as FormArray;
  }

  obtenerCanalesSimulacion() {
    const dataModulo = this.modulos.find(
      (modulo) => modulo.nombre == 'Cliente',
    );
    const dataCategoria = dataModulo?.categorias?.find(
      (categoria) => categoria.nombre == 'Canales Simulador',
    );
    this.sCanalesSimulacion.obtenerTodosTipoCanales().subscribe({
      next: (response) => {
        for (const sub of response.data) {
          dataCategoria!.subcategorias!.push({
            nombre: sub.nombre_canal,
            activo: false,
            permiso: sub.det_canalSimulacion.toLocaleUpperCase(),
          });
        }
      },
  ...
  obtenerEstadosClientes() {
    ...
    const dataCategoria = dataModulo?.categorias?.find(
      (categoria) => categoria.nombre == 'Estados Cliente',
    );
    this.sEstadosCliente.obtenerTodosLosEstados().subscribe({
      next: (response) => {
        for (const sub of response.data) {
          dataCategoria!.subcategorias!.push({
            nombre: sub.nombre_estado,
            activo: false,
            permiso: sub.det_estado.toLocaleUpperCase(),
          });
        }
      },
      error: (error: ErrorHttpCustom) => {
  ...
  combinarPermisos(): PermisosModulo[] {
    const backend = this.usuarioPermisos?.permisos
      ? this.usuarioPermisos.permisos
      : this.modulos;
        ...
        categorias: modulo.categorias?.map((categoria) => {
          ...
          return {
            ...
            subcategorias: categoria.subcategorias?.map((subCategoria) => {
              const subCategoriaBackend =
                categoriaBackend.subcategorias?.find(
                  (cat) => cat.nombre === subCategoria.nombre,
                ) || subCategoria;
